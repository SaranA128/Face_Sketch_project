...
class SketchApp:
    ...
    def _build(self):
        ...
        # TOOLBAR
        tb = tk.Frame(self.root,bg="#f0f5fc",height=52)
        tb.pack(side="top",fill="x"); tb.pack_propagate(False)
        tk.Frame(tb,bg="#00cc55",height=2).pack(side="bottom",fill="x")

        def B(t,cmd,bg2="#e8f0fc",fg2="#00cc55",**kw):
            b=tk.Button(tb,text=t,command=cmd,bg=bg2,fg=fg2,
            ...
            return b
        def SEP(): tk.Frame(tb,bg="#00ff33",width=1).pack(side="left",fill="y",pady=8)

        # Navigation
        B("← Menu",   self._back_menu,  bg2="#f5f9ff", fg2="#546e7a")
        SEP()
        self.bsel=B("↖ Select", lambda:self._set_tool("select"))
        self.bpen=B("✏ Pencil",  lambda:self._set_tool("pencil"))
        self.bers=B("◻ Eraser",  lambda:self._set_tool("eraser"))
        SEP()
        B("↩ Undo",self._undo); B("↪ Redo",self._redo)
        SEP()
        B("↑ Front",self._front); B("↓ Back",self._back_z)
        B("⇄ Flip H",self._flip)
        SEP()
        tk.Label(tb,text="Brush:",bg="#f0f5fc",fg="#1565c0",
                 font=("Courier",8)).pack(side="left",padx=(4,1))
        self.sz=tk.IntVar(value=2)
        tk.Scale(tb,variable=self.sz,from_=1,to=28,orient="horizontal",
                 bg="#f0f5fc",fg="#1565c0",troughcolor="#e8f0fc",
                 highlightthickness=0,length=72,showvalue=True,
                 command=lambda v:setattr(self,"draw_size",int(v))
                 ).pack(side="left")
        tk.Label(tb,text="Opacity:",bg="#f0f5fc",fg="#1565c0",
                 font=("Courier",8)).pack(side="left",padx=(6,1))
        self.op=tk.IntVar(value=100)
        tk.Scale(tb,variable=self.op,from_=10,to=100,orient="horizontal",
                 bg="#f0f5fc",fg="#1565c0",troughcolor="#e8f0fc",
                 highlightthickness=0,length=72,
                 command=self._op_change).pack(side="left")
        SEP()
        B("🗑 Del",   self._delete,     bg2="#ffebee", fg2="#c62828")
        B("Clear",    self._clear,      bg2="#fff3e0", fg2="#e65100")
        SEP()
        B("📂 Open",  self._open_saved, bg2="#e3f2fd", fg2="#0288d1")
        B("💾 Save",  self._save,       bg2="#e8f5e9", fg2="#2e7d32")
        B("🔍 Match", self._compare,    bg2="#e8eaf6", fg2="#0288d1")
    ...
    def _compare(self):
        self.root.destroy(); os.system(f"{sys.executable} upload_match.py")
